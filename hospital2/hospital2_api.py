...
# Relative file paths inside hospital2 folder
BASE_MODEL = "hospital2/main_model.pkl"
...
# Test baseline model on samples (default 3000). Query param ?samples=3000
...
# Train local model (hospital2_v2.pkl) using full TRAIN_DATA
...
def train_local_model():
    ...
    joblib.dump(model, "hospital2_v2.pkl")
    return {"status": "hospital2_v2 trained", "training_samples": len(df)}

# Provide weights to central server (weight-only)
@app.get("/get_weights")
def get_weights():
    try:
        model = joblib.load("hospital2_v2.pkl")
    except Exception as e:
        return {"error": f"hospital2_v2 not found: {e}"}
...
# Test global model on full test set (4,000)
